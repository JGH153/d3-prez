...
d3.json("menu.json").then((data) => {
  ...
  const yAxis = d3.axisLeft(yScaleAxis).ticks(3).tickFormat(i => i + ' Orders');

  xAxisGroup.call(xAxis);
  yAxisGroup.call(yAxis);

  xAxisGroup.selectAll('text').attr("transform", `rotate(40)`).attr('text-anchor', 'start').attr('fill', 'orange');
});
